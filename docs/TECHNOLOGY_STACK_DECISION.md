...
- `core.neutral` package: the initial type system is **seeded systematically from Arcadia semantic coverage and generalized into methodology-neutral engineering concepts**. The baseline spans purpose/mission/objective/capability, participants/entities/systems/roles, activities/functions/behavior, exchanges/interfaces/ports, processes/scenarios/occurrences, state/mode/situation/configuration, environment/context, requirements/constraints/measures, functional/logical/realization structures, allocations, alternatives/variants, analysis/evidence/decision and delivery/lifecycle semantics. Arcadia class names are not copied blindly into the core. Each Arcadia mapping is classified (direct canonical equivalent, canonical composition, profile-only, representation-only, deferred/unsupported). Canonical extensions beyond this baseline require an ADR.
...
| R4 Neutral core vocabulary too thin/thick | Rework of ontology | Slice 1 Arcadia-seeded generalized coverage baseline; ADR discipline; Slice 7 validates profile mappings and adds only justified extensions |
