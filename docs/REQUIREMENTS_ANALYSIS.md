...
Sole input: `CLAUDE_INPUT/` (174 numbered requirement statements across 7 files plus README/vision). No source code of the original project was consulted.
...
3. **Meaning is data, not code.** Methodologies (Arcadia, SysML-style, mission engineering), domains and representation formats are *orthogonal profiles/mappings* over a neutral core (SEM-002, SEM-003, SEM-041, NFR-012, NFR-013). Switching profiles never rewrites confirmed truth (SEM-042).
...
| D3 | **Ontology/profile as data, core neutral** | SEM-002/003/040..043, SEM-050/051, NFR-012..014 | Declarative ontology packages validated against a schema; a generic rule engine with *rule kinds* (not per-methodology branches); vocabulary aliases per audience level; profile (de)activation has no semantic side effects on confirmed data. |
...
| A4 | **The neutral core vocabulary is not defined** - only semantic families are listed (SEM §"Semantic families"). | Risk of a vague "universal ontology" or an accidental single-methodology core. | The core package is **small, generic and derived from the listed families**, with Arcadia used as a *coverage checklist* (not naming source). Grows only when a slice needs a type. Each core type has a one-line meaning and at least one validating example. | Coverage table against Arcadia layers in Slice 7; ADR per new core type. |
...
| A12 | **"Realistic engineering models" (NFR-050) is unquantified.** | Drives store, graph and rendering choices. | See §7 targets. | Benchmark spike (Slice 0). |
