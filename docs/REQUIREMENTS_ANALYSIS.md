...
Sole input: `CLAUDE_INPUT/` (the compact numbered requirement baseline plus the normative clarifications in `08_ARCADIA_CANONICAL_SEED.md` and `09_CONVERSATION_UI_ANY_LEVEL_ENTRY.md`). No source code of the original project was consulted.
...
5. **Low cognitive load is a measurable design objective** (UX-070, NFR-060..062): a modern conversational-assistant interaction pattern with one dominant conversation, one obvious next action, minimal persistent chrome, compact "what I understand", and contextual/on-demand model detail. The user may begin at any engineering abstraction level; the system detects canonical/methodology gaps around the known state and chooses questions dynamically rather than enforcing a top-down wizard.
...
| D6 | **Conversation-like minimal UI + contextual model + any-level entry** | REQ-CF-*, UX-*, `09_CONVERSATION_UI_ANY_LEVEL_ENTRY.md` | Modern conversational-assistant pattern: conversation visually dominant, minimal persistent controls, compact canonical-model context secondary, details/actions exposed through hover/click/context-menu/popover/drawer patterns. The planner must accept starting knowledge at any abstraction level and select upstream/downstream/lateral gap questions from current semantic state rather than a fixed lifecycle sequence. |
...
| A10 | **"Continuously inspectable" (VIEW-010) vs "no accumulating permanent panels" (REQ-CF-007, UX-071)** | UI layout. | **Resolved by ADR-0004:** conversation is visually dominant; a compact canonical-model/context surface may remain secondary; deeper model information is contextual/on-demand through hover, selection, context menu, popover/drawer/dialog or expanded lens. Closing secondary detail restores the pending primary action. | Human validation (UX-070), including clutter/context-switch measures. |
...
| A18 | **Must projects start top-down from mission/challenge?** | A fixed lifecycle wizard would fail brownfield and solution-first engineering use cases. | **Resolved by ADR-0004:** no. The user may begin at any supported abstraction level, including realization/physical architecture. The system interprets the starting state, then computes canonical and methodology gaps upstream, downstream and laterally and asks the most useful next question. | Slice 1 orchestration contract + later human validation with challenge-first, requirement-first and physical-solution-first scenarios. |
