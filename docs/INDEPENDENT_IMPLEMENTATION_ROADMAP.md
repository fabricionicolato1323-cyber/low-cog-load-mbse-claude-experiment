...
**User story:** *I may start with an engineering challenge or with known engineering content at another abstraction level (including an existing physical/realization solution). The system tells me what it understands, identifies gaps around that known state, asks the single most useful next question, and lets me confirm/correct answers; everything persists and can be explained.*
...
- App: `NextActionPlanner` (deterministic, one primary action + <= 3 alternates + `why`) driven by current semantic state rather than lifecycle position. Its input contract must support a known starting point at any abstraction/context and allow upstream, downstream and lateral information needs. Conversation history includes "what changed"; compact **Understanding summary** supports confirm/reject/refine on candidates (no IDs shown).
- Store: SQLite adapter (log + materialised + candidates + conversation).
- API + UI: **modern conversational-assistant shell** with conversation visually dominant, minimal persistent controls, compact understanding/canonical-model context secondary, pending-candidate confirmation, and disclosure levels (Normal/Advanced/Expert; Expert shows IDs/provenance read-only). Define contextual model-detail/action contracts so later graph elements can expose hover/select/context-menu/popover/drawer detail without adding permanent panels.
...
**Acceptance scenarios:** (1) start from an engineering challenge, answer guided questions, close and reopen -> identical state; (2) start from a downstream/realization-oriented statement and verify the planner can identify missing upstream/lateral information without forcing restart from mission; (3) failed validation leaves state untouched; (4) two elements with identical names remain distinct; (5) correcting a statement produces a new ChangeSet, history preserved; (6) "why are you asking this?" returns a rule/need-based explanation; (7) Normal Mode remains conversation-dominant with secondary details disclosed on demand.
